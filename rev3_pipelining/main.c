...
// Function to calculate SAD for a block with software pipelining
unsigned short calculate_sad(unsigned char *current, unsigned char *reference, int x, int y, int ref_x, int ref_y, int width, int height) {
    ...
    // Prologue: Handle the first few iterations to start filling the pipeline
    ...
    // Steady-state loop: Main loop with pipelining
    for (int i = 0; i < BLOCK_SIZE; i++) {
        for (int j = 2; j < BLOCK_SIZE - 2; j++) {
            int cur_index0 = (y + i) * width + (x + j - 2);
            int ref_index0 = (ref_y + i) * width + (ref_x + j - 2);
            int cur_index1 = (y + i) * width + (x + j - 1);
            int ref_index1 = (ref_y + i) * width + (ref_x + j - 1);
            int cur_index2 = (y + i) * width + (x + j);
            int ref_index2 = (ref_y + i) * width + (ref_x + j);
            int cur_index3 = (y + i) * width + (x + j + 1);
            int ref_index3 = (ref_y + i) * width + (ref_x + j + 1);
            int cur_index4 = (y + i) * width + (x + j + 2);
            int ref_index4 = (ref_y + i) * width + (ref_x + j + 2);

            if (cur_index4 < width * height && ref_index4 < width * height) {
                ...
                sad += abs(current[cur_index4] - reference[ref_index4]);
            }
        }
    }

    // Epilogue: Handle the last few iterations to drain the pipeline
    for (int i = 0; i < BLOCK_SIZE; i++) {
